...
## Схема данных и их поток

```mermaid
flowchart TD
    subgraph Input["🎯 Входные данные"]
        A[GPS Coordinates<br>from Device]
        B[User Interactions<br>Click/Scroll]
        C[Configuration<br>Settings]
    end
    ...
    subgraph Output["📊 Выходные данные"]
        I[UI Updates<br>LiveData]
        J[File Logs<br>JSON/Text]
        K[Cache<br>Memory/Disk]
...
Схема показывает как входные данные (координаты GPS, действия пользователя) проходят через различные этапы обработки и преобразуются в выходные данные (обновления UI, логи, кэш). Подчеркивает сквозной поток информации через все слои приложения.

**Входные данные:**
- GPS Coordinates - географические координаты от устройства пользователя.
- User Interactions - жесты и действия пользователя в интерфейсе.
- Configuration - настройки приложения и параметры системы.

**Обработка данных:**
- Location Resolution - преобразование координат в читаемый адрес.
- Network Request - выполнение запросов к серверу.
- Data Parsing - преобразование JSON в объекты Java.
- Business Logic - применение правил и алгоритмов к данным.
- Error Handling - обработка исключительных ситуаций.

**Выходные данные:**
- UI Updates - обновление интерфейса через LiveData/StateFlow.
- File Logs - запись информации для отладки и анализа.
- Cache - временное хранение для оптимизации производительности.
- User Notifications - уведомления пользователя о событиях.

## Схема API эндпоинтов

```mermaid
graph LR
    subgraph API["🌐 Flask API Endpoints"]
        A1[GET /api/current<br>📊 Текущая погода]
        A2[GET /api/forecast<br>📈 Прогноз погоды]
        A3[POST /api/logs<br>📝 Логи клиента]
        A4[GET /status<br>❤️ Health check]
    end
    
    subgraph Responses["📨 Ответы API"]
        B1[JSON<br>Temperature, Humidity, Wind]
        B2[JSON<br>Hourly/Daily Forecast]
        B3[HTTP 200<br>Log accepted]
        B4[JSON<br>Server status]
    end
    ...
    A4 --> B4
```
Схема, фокусирующаяся на REST API интерфейсе сервера. Перечисляет все доступные эндпоинты с их назначением и форматом возвращаемых данных. Полезна для разработчиков как клиентской, так и серверной части, обеспечивая контракт взаимодействия.

**Эндпоинты API:**
- GET /api/current - возвращает текущую погоду для указанных координат.
- GET /api/forecast - предоставляет прогноз погоды на несколько дней.
- POST /api/logs - принимает логи от клиентского приложения.
- GET /status - проверка работоспособности сервера.

**Форматы ответов:**
- JSON для данных - стандартный формат для структурированных данных.
- HTTP статусы - индикаторы успеха/ошибки выполнения запроса.
- Единая структура - согласованный формат всех ответов API.

## Схема обработки ошибок

```mermaid
graph TD
    A[🔄 Start Request] --> B{🌐 Network Available?}
    B -->|No| C[📱 Show Offline Message]
    B -->|Yes| D[🚀 Send API Request]
    
    D --> E{✅ Response OK?}
    E -->|No| F{🔍 Error Type?}
    E -->|Yes| G[🎉 Update UI with Data]
    
    F -->|Timeout| H[⏰ Show Retry Button]
    F -->|Server Error| I[🔧 Show Service Message]
    F -->|Invalid Data| J[🔄 Retry Request]
...
```
Диаграмма принятия решений, которая детально описывает стратегию обработки различных типов ошибок в приложении. Показывает различные сценарии failures (проблемы сети, ошибки сервера, невалидные данные) и соответствующие действия системы. Демонстрирует robust-подход к обработке исключительных ситуаций.

**Типы ошибок:**
- Network Available - проверка подключения к интернету.
- Response OK - валидация успешности HTTP-запроса.
- Timeout - обработка превышения времени ожидания.
- Server Error - ошибки на стороне сервера (5xx).
- Invalid Data - невалидные или некорректные данные.

**Стратегии восстановления:**
- Show Offline Message - информирование пользователя об отсутствии сети.
- Show Retry Button - предоставление возможности повтора запроса.
- Contact Support - направление в службу поддержки для критических ошибок
- Cache Data - использование кэшированных данных при недоступности сети.
- Return to Home - безопасный возврат к основному функционалу.
